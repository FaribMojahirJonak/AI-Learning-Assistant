...
const UserIcon = () => (
  <span className="userprofile-avatar" style={{display:'inline-flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.7)',boxShadow:'0 2px 8px rgba(161,196,253,0.10)'}}>
    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="12" cy="8" r="6" fill="#a1c4fd"/>
      <ellipse cx="12" cy="18" rx="9" ry="5" fill="#bdb4fe"/>
    </svg>
  </span>
);
...
export const UserProfile: React.FC = () => {
  ...
  if (!user) return <div style={{textAlign:'center',marginTop:'40px'}}>Please sign in to view your profile.</div>;
  if (loading) return <div style={{textAlign:'center',marginTop:'40px'}}>Loading your learning history...</div>;
            ...
            <div className="userprofile-stat-row">
              <span className="userprofile-stat-label">
                <span className="userprofile-stat-icon">
                  <svg width="16" height="16" fill="none" viewBox="0 0 20 20"><rect x="3" y="4" width="14" height="12" rx="3" fill="#2563eb" fillOpacity="0.18"/><rect x="6" y="7" width="8" height="2" rx="1" fill="#2563eb"/><rect x="6" y="11" width="5" height="2" rx="1" fill="#2563eb"/></svg>
                </span>
                Total Lessons:
              </span>
              <span className="userprofile-stat-value">{totalLessons}</span>
            </div>
            <div className="userprofile-stat-row">
              <span className="userprofile-stat-label">
                <span className="userprofile-stat-icon">
                  <svg width="16" height="16" fill="none" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="#2563eb" fillOpacity="0.18"/><path d="M10 6v4l3 2" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </span>
                Total Quizzes:
              </span>
              <span className="userprofile-stat-value">{totalQuizzes}</span>
            </div>
            <div className="userprofile-stat-row">
              <span className="userprofile-stat-label">
                <span className="userprofile-stat-icon">
                  <svg width="16" height="16" fill="none" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="#2563eb" fillOpacity="0.18"/><path d="M6 10l2.5 2.5L14 7" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </span>
                Average Score:
              </span>
              <span className="userprofile-stat-value">{avgScore}%</span>
            </div>
          </div>
        </div>
        {/* Right: Lessons */}
        <div className={`userprofile-right${lessons.length === 0 ? ' empty' : ''}`}>
              ...
              >
                ...
                <div className="userprofile-progress">
                  <svg width="64" height="64" viewBox="0 0 64 64">
                    <circle
                      className="userprofile-progress-bg"
                      cx="32" cy="32" r="28"
                      fill="none"
                    />
                    <circle
                      className={`userprofile-progress-fg ${level}`}
                      cx="32" cy="32" r="28"
                      fill="none"
                ...
                <div className="userprofile-card-actions">
                  ...
                  <button
                    className="userprofile-btn delete"
          ...
          {lessons.length === 0 && (
            <div className="userprofile-no-lessons">
              You haven't created any lessons yet.<br />Start learning a new topic to see your progress here!
            </div>
          )}
        </div>
      </div>
